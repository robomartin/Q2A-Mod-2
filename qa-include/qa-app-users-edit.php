...
*/
	{
		...
		// ******************************************************************************* START MODIFICATION
		// Create an obfuscated password where we only show the last character
		if(isset($password))
		{
			$obfuscated_password = str_repeat("*", strlen($password) - 1) . substr($password, -1);
		}
		// ******************************************************************************* END MODIFICATION

		qa_send_notification($userid, $email, $handle, qa_lang('emails/welcome_subject'), qa_lang('emails/welcome_body'), array(
		// ******************************************************************************* START MODIFICATION
		// For security reasons, don't want to email plain-text password
//			'^password' => isset($password) ? $password : qa_lang('users/password_to_set'),
			'^password' => isset($password) ? $obfuscated_password : qa_lang('users/password_to_set'),
		// ******************************************************************************* END MODIFICATION
			'^url' => qa_opt('site_url'),
